...
T-010,G-001,T-080,在 src/App.tsx 的手動存檔區塊新增「匯出存檔」按鈕：點擊後將目前 gameState 序列化為格式化 JSON 並以 Blob + URL.createObjectURL 觸發下載檔名 xianxia-save.json；另在 src/utils/save.ts 新增匯出用 helper（例如 exportSaveData）封裝資料轉換與檔名常數，並沿用現有 save 工具命名與匯出風格,Coder-Bot,waiting
